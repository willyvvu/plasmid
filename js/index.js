...
(function() {
	try {
		var Plasmid = function(canvascontainer) {
			...
			this.text = {
				...
				counterlabel: {
					...
					width: 350,
					position: new THREE.Vector3(0, -165, 151), //Where to place the text
					repeat: new THREE.Vector2(1, 1 / 3),
					map: "counter",
					opacity: function() {
						return (this.state & this.INLEVEL) != 0 ? 1 : 0;
				...
				help: {
					...
					handler: function() {
						if (this.state == this.MAIN) {
							window.location = "https://github.com/willy-vvu/plasmid#how-to-play";
						}
					}
				}
			}
			...
			this.credits = {
				...
				duration: 10, //How many seconds it should take
			}
		...
		Plasmid.prototype = {
			...
			update_ui: function(override_lerpspeed) {
				...
				if (this.state == this.MAIN) {
					...
					this.lerp_add(this.shaders.desaturate.uniforms.amount, "value", 0, lerpspeed);
				}
				if ((this.state & this.INLEVEL) != 0) {
					//In a level
			...
			loop: function() {
				...
				if (this.credits._scroll > 0) {
					if (this.credits.scroll < 1) {
						...
						this.level.object.position.y = 500 * Math.pow(2 * this.credits.scroll - 1, 5) + 100 * (2 * this.credits.scroll - 1);
					} else {
						//Finish up
				...
				if (this.state != this.LOADING) {
					var pulse = Math.sin(this.currentTime * 0.003) * 0.003;
					//Pulse the opacity of the campaign button
					if (this.text.campaign.opacity.call(this) > 0) {
						this.text.campaign.material.opacity = this.utils_clamp(this.text.campaign.material.opacity + pulse * 5, 0, 1);
					}
					//Pulse the opacity of the undo/reset buttons
					if (this.text.undo.opacity.call(this) > 0 && this.current.generation >= this.current.level.generations) {
						this.text.undo.material.opacity = this.text.reset.material.opacity = this.utils_clamp(this.text.undo.material.opacity + pulse * 5, 0, 1);
					}
				}
				//Render!
				if (this.usePostProcess) {
		...
		p.canvascontainer.addEventListener("touchstart", function(event) {
			...
			if ("requestFullScreen" in p.canvascontainer) {
				p.canvascontainer.requestFullScreen();
			} else if ("webkitRequestFullScreen" in p.canvascontainer) {
				p.canvascontainer.webkitRequestFullScreen();
			} else if ("mozRequestFullScreen" in p.canvascontainer) {
				p.canvascontainer.mozRequestFullScreen();
			}
		...
		function loop() {
			try {
				...
				if (confirm("Oh no! Nondisjunction! View the troubleshooting page?\n" + e)) {
					window.location = "https://github.com/willy-vvu/plasmid#troubleshooting"
				};
			}
		}
		loop();
